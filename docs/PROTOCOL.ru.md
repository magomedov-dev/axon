...
## 7. Запланированные методы 🔜

Зарезервированы и описаны здесь для полноты; пока не реализованы.

- **`setEventStream`** — `{ enabled: bool }` включает server-push для этого соединения.

### Запланированные server-push события 🔜
