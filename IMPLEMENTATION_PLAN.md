...
> `dumpHierarchy`, `getWindows`, `gesture`, `nodeAction`, `globalAction`,
> `screenshot`, `setEventStream` + события (`screenChanged`/`toast`). Покрытие:
> JVM unit-тесты + E2E-стадии на устройстве (`scripts/test.sh`). Документация —
> двуязычная (RU/EN).
...
- Чистый парсер `screenshot/ScreenshotRequest.kt`. Атомарная отправка двух
  сообщений реализована каналом `RpcContext.binary` → диспетчер вызывает
  `FrameWriter.sendJsonThenBinary` (один `Mutex`). `Agent.captureScreenshot`
  оборачивает `takeScreenshot` (HardwareBuffer→Bitmap→компрессия).

**Готово ✅:** JPEG и PNG приходят как метаданные + бинарь; заголовок = id запроса;
длина = `bytes`; магические байты валидны; `INVALID_PARAMS` на плохой формат;
`INTERNAL` при сбое/лимите. Проверено: 6 unit-тестов `ScreenshotRequestTest` +
E2E `stage6_screenshot` (9 проверок, JPEG+PNG).
...
- Тумблер `MaterialSwitch` (`setServerEnabled`), живое обновление статуса каждые 1с
  (счётчик соединений). Попутно найдена и исправлена реальная гонка: старт/стоп
  сервера сериализованы на одном lifecycle-потоке (быстрый toggle off→on больше не
  оставляет «доживающий» сервер на том же порту).

**Готово ✅:** индикаторы сервиса/сервера, живой счётчик подключений, тумблер реально
стартует/останавливает сервер, кнопка настроек, переключатель языка. Проверено
(скриншот + E2E `stage9_console`, 8 проверок): индикаторы, счётчик 1→2→1, тумблер
останавливает сервер и сервер восстанавливается после рестарта сервиса.
...
**Готово ✅:** протокол полностью задокументирован (оба языка); `getWindows`
**реализован** (перечисление всех окон + опц. дерево каждого через `includeTree`,
`tree/WindowJson.kt` + `GetWindowsHandler`); сквозной сценарий проходит. Проверено:
E2E `stage10_scenario` (10 проверок в одном потоке: ping → dump → gesture+toast →
nodeAction → screenshot → globalAction+screenChanged → ping) + `stage11_getwindows`
(метаданные окон, наше app-окно, includeTree с деревом) + unit `WindowJsonTest`.
