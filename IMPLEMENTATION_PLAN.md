...
**Готово:** `dumpHierarchy` возвращает валидное дерево реального экрана; `maxDepth`
и `compress` работают; корень содержит `screen` и `package`; на пустом корне —
внятный error.

**Проверка вручную:** открыть любое приложение, вызвать `dumpHierarchy`, сверить
узлы/bounds/center с экраном; проверить `maxDepth:1` и `compress:true`.
...
**Готово:** одиночный тап, свайп и мультитач отрабатывают; ответ приходит после
фактического завершения жеста; превышение лимита длительности — внятный error.

**Проверка вручную:** тап по координате кнопки (сверить с `dumpHierarchy`), свайп-
скролл, проверить что ответ приходит ~через duration, а не мгновенно.

**Зависимости:** Этап 1 (можно после Этапа 2 для удобства проверки координат).
...
**Готово:** click/setText/scroll по реальным узлам работают; неоднозначность,
отсутствие узла, не-editable, неподдерживаемый action и stale — каждый со своим
внятным кодом; `index` выбирает нужный из нескольких.

**Проверка вручную:** click по кнопке, setText в поле, scrollForward в списке;
спровоцировать каждую ошибку (несуществующий resourceId, setText без text и т.д.).

**Зависимости:** Этап 2 (NodeModel/обход), Этап 1.
...
**Готово:** back/home/recents/notifications и пр. выполняются; неизвестное действие —
внятный error.

**Проверка вручную:** вызвать каждое действие, наблюдать поведение устройства.
