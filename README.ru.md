...
- Вызовы, зависящие от дерева, возвращают `ACCESSIBILITY_DISABLED` (без падения),
  ...
  unbind/destroy.
- FGS-*тип* `specialUse` обязателен на API 34+; на API 30–33 он игнорируется, но
  сервис всё равно промоутится (проверено на API 33). Разношёрстный парк
  (API 30–34+) стоит держать в тест-матрице.
...
Экран приложения — живой пульт: индикаторы сервиса/сервера, счётчик подключений,
тумблер запуска/остановки сервера и переключатель языка RU/EN.
